...
static void gpuCrossprodIntern(unsigned int* CGM, size_t snps, size_t individuals,
  ...
  for (int64_t i = 0; i < individuals; i += TileSize) {
      ...
      cutlass::uint4b_t *d_tile1 = d_Z_block1 + threadidx * TileSize * BytesPerRow ;
      cutlass::uint4b_t *d_tile2 = d_Z_block2 + threadidx * TileSize * BytesPerRow ;
     ...
     cudaMemcpyAsync(d_tile1, x, x_tile_size * BytesPerRow,cudaMemcpyHostToDevice, stream);
      ...
      for (int64_t j = i; j < individuals; j += TileSize) {
         ...
         cudaMemcpyAsync(d_tile2, y, y_tile_size * BytesPerRow, cudaMemcpyHostToDevice, stream);
          err_check("Copy 2:");
          cudaStreamSynchronize(stream);
...
#pragma omp critical
          ...
          cudaMemcpyAsync(h_M + threadidx * TileSize * TileSize, d_M + threadidx * TileSize * TileSize, TileSize * TileSize * sizeof(int32_t), cudaMemcpyDeviceToHost, stream);
          err_check("Copying back:");
          
          cudaStreamSynchronize(stream);

          if(*(h_M + threadidx * TileSize * TileSize)==0){printf("Computation failed at thread %d, (%d,%d)\n",threadidx, i,j);
                      print_kernel <<<1,1>>> ((int32_t*) d_M + threadidx * TileSize * TileSize);
              j-=TileSize;
              continue;
          }
          err_check("Copy back:");
          ...
          for (int64_t di = 0; di < x_tile_size; ++di)
          {
              ...
              {
                  // Get result
                  const auto Mij = *(h_M +threadidx * TileSize * TileSize + dj + di * y_tile_size ); 
