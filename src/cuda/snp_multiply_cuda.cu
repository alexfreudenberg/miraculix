...
int snp_crossproduct(unsigned char *snp_matrix, int snps, int indiv,
  ...
  cudaError_t err = cudaSuccess, private_err = cudaSuccess;
  cudaStream_t stream;
  ...
  const long n_bytes_per_indiv_padded = (1 + (n_bytes_per_indiv - 1) / 32) * 32;
  ...
  debug_info("Function arguments: snps %d, indiv %d, bytes_per_indiv %d, "
             "bytes_per_indiv_padded %d, k %d.\n",
             snps, indiv, n_bytes_per_indiv, n_bytes_per_indiv_padded,
             n_bytes_per_indiv_padded * n_snps_per_u4b);
  ...
  size_t required_mem =
      num_threads * (2 * size_of_input) + num_threads * size_of_output;
...
#ifdef DO_PARALLEL
#pragma omp parallel for num_threads(num_threads) private(private_err, stream) \
    shared(err) schedule(dynamic)
#endif
  for (long i = 0; i < indiv; i += mem_tile_size) {
    ...
    cutlass::uint4b_t *d_tile1 =
        (cutlass::uint4b_t *)(d_Z_block1 + threadidx * mem_tile_size *
                                               n_bytes_per_indiv_padded);
    cutlass::uint4b_t *d_tile2 =
        (cutlass::uint4b_t *)(d_Z_block2 + threadidx * mem_tile_size *
                                               n_bytes_per_indiv_padded);
    ...
    int x_tile_size_padded = ((x_tile_size - 1) / 4 + 1) *
                             4; // padded tile sizes are required as CUTLASS
                                // needs dimensions that are a multiple of 4

    private_err = cudaMemcpy2DAsync(
        d_tile1, n_bytes_per_indiv_padded, x, n_bytes_per_indiv,
        n_bytes_per_indiv, x_tile_size, cudaMemcpyHostToDevice, stream);
    ...
    if (is_plink_format) {
      device_convert_plink_2bit((unsigned char *)d_tile1,
                                n_bytes_per_indiv_padded * x_tile_size,
                                functor);
    }
    ...
    for (long j = i; j < indiv; j += mem_tile_size) {
      ...
      int y_tile_size_padded = ((y_tile_size - 1) / 4 + 1) *
                               4; // padded tile sizes are required as CUTLASS
                                  // needs dimensions that are a multiple of 4

      private_err = cudaMemcpy2DAsync(
          d_tile2, n_bytes_per_indiv_padded, y, n_bytes_per_indiv,
          n_bytes_per_indiv, y_tile_size, cudaMemcpyHostToDevice, stream);
      ...
      if (is_plink_format) {
        device_convert_plink_2bit((unsigned char *)d_tile2,
                                  n_bytes_per_indiv_padded * y_tile_size,
                                  functor);
      }
      private_err = cudaGetLastError();
      if (checkError(__func__, __LINE__, private_err) != 0) {
...
int snp_multiply_gpu(unsigned char *snp_matrix, int snps, int indiv,
                      double *ans, bool is_plink_format) {
    return snp_crossproduct(snp_matrix, snps, indiv, ans, is_plink_format);
}
