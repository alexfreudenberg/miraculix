...
#define bits 2L
#define blocklength 8L
#define TWO_BITSPERBLOCK 65536L // INFOLAST^(bits * blocklength)

static double rev_geno_code[4] = {0, 1, RF_NA, 2}; 
static uint64 geno_code[nr_genotypes] = {0, 1, 3}, // 0, 1, 2
...
static table_type *TABLE2AND = NULL,
  *TABLE2XOR = NULL;
static Uint TABLE2_SIZE = 0;
void initiate_table2() { // hash table 
  if (TABLE2AND != NULL || TABLE2XOR != NULL) BUG; 
  initiate_tableI(&TABLE2AND, &TABLE2_SIZE, blocklength, bits, result_code,
		  result_valueAND, nr_results);
  initiate_tableI(&TABLE2XOR, &TABLE2_SIZE, blocklength, bits, result_code,
		  result_valueXOR, nr_results);
}
...
SEXP matrix_coding_start2 matrix_coding_start(TABLE2_SIZE)



// BODY-CODE IDENTICAL TO CODING3
void matrix_coding2(Uint *M, Uint start_individual, Uint end_individual, 
  ...
  //  printf("%d %d %d \n", start_snp,n_compressed, snpsPerCompressed);
...
void MmPsq2(Uint individuals, Uint min_row, Uint max_row,
  ...
  table_type table[65536], tableXor[65536];
  MEMCOPY(table, TABLE2AND, 65536 * sizeof(table_type));
  MEMCOPY(tableXor, TABLE2XOR, 65536 * sizeof(table_type));
