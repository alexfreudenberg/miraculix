...
void gV_vG_means_LongDouble(SEXP SxI, 
			LongDouble *V0, Long repetV, Long ldV, 
			bool gV, // else vG
...
#pragma omp parallel for num_threads(cores) schedule(static) 
  ...
  if (meanV) {
    V = (LongDouble*) MALLOC(repetV * ldV * sizeof(*V));
#ifdef DO_PARALLEL
#pragma omp parallel for num_threads(cores) schedule(static) 
#endif
    for (Long r=0; r < repetV; r++) {
      LongDouble *vv = V + r * ldV;
      LongDouble *vv0 = V0 + r * ldV;
      LongDouble mu0 = 0;
      ...
      mu[r] = mu0;
      //   LongDouble mu1 = (LongDouble) mu0;
      for (Long i=0; i<cols; i++) vv[i] = (LongDouble) ((LongDouble) vv0[i] - mu0);
    }
  }
  if (PL && meanOrCZ) CLOCK(meanV ? "meanV" : "cMinusZ");
  
  gV_vG_LongDouble(SxI, V, repetV, ldV,
...
#pragma omp parallel for num_threads(cores) schedule(static) 
	...
	freqFactor *= -2.0L;
      ...
      //      // printf("c-z=%4.3e frqV=%4.3e c=%4.3e nu=%4.3e mu=%4.3e pF=%4.3e factor=%4.3e,%4.3e \n",(LongDouble) cMinusZ, (LongDouble) freqV[r], (LongDouble) c , (LongDouble) nu , (LongDouble)  mu[r], (LongDouble) pseudoSumFreq, (LongDouble) freqFactor, (LongDouble) onesFactor );
      
      for (Long j=0; j<rows; j++) {
	// hier geht gegenueber LongDouble die Genauigkeit verloren:
	// wenn in *LD.cc  (LongDouble) (LongDouble) ((LongDouble) (LongDouble) a[j]
	// gesetzt wird ist selbst bei Centered kein Unterschied mehr da!

	a[j] = (LongDouble) ((LongDouble) a[j] + sxiOneFactor * sxiOne[j] +
...
#pragma omp parallel for num_threads(cores) schedule(static) 
	 ...
	 for (Long i=0; i<end_i; i+=2) {
	   ...
	   a[alpha] = (LongDouble) ((LongDouble) a[alpha] +
				(Z * freq[alpha] + Zeta * pseudofreq[beta])
				* 2.0L * (LongDouble) vv0[beta]);
	 }
	 //   // printf("(LongDouble) a[0]=%4.3f\n", (LongDouble) a[0]);
       }
   ...
   if (normalized == GlobalNormalizing) {
     ...
     LongDouble sigma = (LongDouble) sqrtl(sigmaSq);
#ifdef DO_PARALLEL
#pragma omp parallel for num_threads(cores) schedule(static) 
#endif
     for (Long r=0; r < repetV; r++) {
...
void genoVector_means_LongDouble(SEXP SxI, 
			     LongDouble *V, Long repetV, Long ldV, 
			     option_type *global, utilsoption_type *utils,
			     LongDouble *ans, Long ldAns) {
         gV_vG_means_LongDouble(SxI, V, repetV, ldV, 
		     true,
...
void genoVector_means_LongDouble(SEXP SxI, 
			     LongDouble *V, Long repetV,
			     option_type *global, utilsoption_type *utils,
			     LongDouble *ans) {
  Long *info = GetInfo(SxI);
  genoVector_means_LongDouble(SxI, V, repetV, info[INDIVIDUALS], 
			  global, utils, ans, info[SNPS]);
}

void vectorGeno_means_LongDouble(SEXP SxI, 
			     LongDouble *V, Long repetV, Long ldV, 
			     option_type *global, utilsoption_type *utils,
			     LongDouble *ans, Long ldAns) {
  gV_vG_means_LongDouble(SxI, V, repetV, ldV,
...
void vectorGeno_means_LongDouble(SEXP SxI, 
			     ...
			     LongDouble *ans) {
  Long *info = GetInfo(SxI);
  vectorGeno_means_LongDouble(SxI, V, repetV, info[SNPS],
