...
int main(void) {
    ...
    int n = 10000;
    const int A_num_rows = n;
    const int A_num_cols = n;
    const int A_nnz      = n + 200;
    const int nrhs       = 20;
    ...
    int   *hA_coo_rows, *hA_coo_cols;
    double *hA_values, *hB, *hC;
    int   *dA_coo_rows, *dA_coo_cols;
    double *dA_values, *dB, *dC;
    CHECK_CUDA( cudaMallocHost((void**) &hA_coo_rows, A_nnz * sizeof(int))      )
    CHECK_CUDA( cudaMallocHost((void**) &hA_coo_cols, A_nnz * sizeof(int))      )
    CHECK_CUDA( cudaMallocHost((void**) &hA_values,   A_nnz * sizeof(double))    )
    CHECK_CUDA( cudaMallocHost((void**) &hB, nrhs * A_num_cols * sizeof(double)) )
    CHECK_CUDA( cudaMallocHost((void**) &hC, nrhs * A_num_rows * sizeof(double)) )

    for(int i = 0; i < A_nnz; i++){
        hA_coo_rows[i] = rand() % A_num_rows;
        hA_coo_cols[i] = rand() % A_num_rows;
        hA_values[i] = (double) rand() / (double) RAND_MAX;
    }
    for(int i = 0; i < nrhs * A_num_cols; i++){
        hB[i] = (double) rand() / (double) RAND_MAX;
    }
    ...
    CHECK_CUDA( cudaMalloc((void**) &dB, nrhs * A_num_cols * sizeof(double)) )
    CHECK_CUDA( cudaMalloc((void**) &dC, nrhs * A_num_rows * sizeof(double)) )
    ...
    CHECK_CUDA( cudaMemcpy(dA_values, hA_values, A_nnz * sizeof(double),
                           cudaMemcpyHostToDevice) )
    CHECK_CUDA( cudaMemcpy(dB, hB, nrhs * A_num_cols * sizeof(double),
                           cudaMemcpyHostToDevice) )
    CHECK_CUDA( cudaMemcpy(dC, hC, nrhs * A_num_rows * sizeof(double),
                           cudaMemcpyHostToDevice) )
    
    CHECK_CUDA(cudaMemset(dC, 0.0, A_num_cols * nrhs))
    //--------------------------------------------------------------------------
    // CUSPARSE APIs
    ...
    //--------------------------------------------------------------------------
    // device result check
    CHECK_CUDA( cudaMemcpy(hC, dC, nrhs * A_num_rows * sizeof(double),
                           cudaMemcpyDeviceToHost) )
    int correct = 1;
    // for (int i = 0; i < nrhs * A_num_rows; i++) {
    //     if (hC[i] != hY_result[i]) { // direct doubleing point comparison is not
    //         correct = 0;             // reliable
    //         break;
    //     }
    // }
