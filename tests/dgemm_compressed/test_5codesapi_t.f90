program test_5codesAPI_t
 ...
 allocate(c_B(mat%ngen, ncol))
 allocate(c_C(mat%nsnp, ncol))
 ...
 !$ t1 = omp_get_wtime()
 call c_setOptions_compressed(usegpu, 0, 0, 0, 1, c_not_center, 0, 0, 0, c_lverbose)
...
pure subroutine initarrays(B, C)
 ...
 B = reshape([((-(1000._real64*i+j), i=1, size(B, 1)), j=1, size(B, 2))]&
             , [size(B, 1), size(B, 2)])

 C = reshape([((-(1000._real64*i+j), i=1, size(C, 1)), j=1, size(C, 2))]&
             , [size(C, 1), size(C, 2)])
...
pure subroutine initarrays_c(B, C)
 ...
 B = reshape([((-(1000._c_double*i+j), i=1, size(B, 1)), j=1, size(B, 2))]&
             , [size(B, 1), size(B, 2)])

 C = reshape([((-(1000._c_double*i+j), i=1, size(C, 1)), j=1, size(C, 2))]&
             , [size(C, 1), size(C, 2)])
