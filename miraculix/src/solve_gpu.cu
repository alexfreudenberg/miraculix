...
// /*__global__ void print_kernel(int32_t* d_C, int individuals, int snps) {
//     for(int i = 0; i < individuals * snps;i++) printf("% " PRId32, d_C[i] );
// }

// __global__ void print_kernel(int8_t* d_C, int individuals, int snps) {
//     for(int i = 0; i < individuals * snps;printf( (++i % individuals) ? "" : "\n")   ) printf("% " PRId8, d_C[i] );
// }
// */
...
//     PRINTF("Code %i\n", h_info);
//     if (0 != h_info) {
//         ERR("Error: Cholesky factorization failed\n");
//     }
// //calculate x = A\b
//     checkCudaErrors(
